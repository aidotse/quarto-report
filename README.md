...
themeable **HTML** site with working light/dark mode. It packages the mechanics
...
Requires a recent [Quarto](https://quarto.org) (Typst ships with it). Fonts are
...
columns). Do **not** use mdformat or Prettier — they are CommonMark tools that corrupt
...
- **on every commit** — `panache format` (auto-formats staged `.qmd`/`.md`) and
  `panache lint` (broken references, heading hierarchy). Milliseconds.
...
verify without writing). The pre-push render test is local convenience and bypassable
...
mapping breaks. This dependency-free bash smoke test guards that seam. It renders
...
HTML site to GitHub Pages. The workflow assumes the template is the **repository
root**, so:
...
- **Typst heading levels are positional**, not absolute (see `style.typ`): the
  ...
  it has. Keep your heading hierarchy consistent.
- **Edit the cover text in both branches** of `index.qmd` — the Typst page and the
  HTML hero are independent.
- **Fonts** are bundled; `style.typ` uses Roboto + Roboto Mono. Inter is also in
...
1. **Branding:** the logo is the **AI Sweden** wordmark, shipped in two variants —
   ...
   dark cover/hero). The **hero background** (`hero-bg.svg`) and the demo
   `diagram-*.svg` / `chart.svg` are generic, license-clean placeholders — swap
   them for your own. Filenames referenced by `index.qmd` / `styles.css`:
